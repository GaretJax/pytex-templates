\documentclass[9pt,a4paper,twoside,onecolumn]{article}
...
% Allows linebreaks in underlined text (use \ul to underline)
\usepackage{soul}

% Allow sections of multiple columns
\usepackage{multicol}
...
\newcommand{\addinvitee}[2]{\listadd{\invitees}{#1}\listadd{\inviteemails}{#1 \email{#2}}}
\newcommand{\fmtinvitee}[2]{#1 #2}
\newcommand{\setdatetime}[2]{\def\date{#1}\def\time{#2}}
\newcommand{\setvenue}[1]{\def\venue{#1}}
\newcommand{\setproject}[1]{\def\project{#1}}
\newenvironment{agenda}{%
	\usection{Agenda}
	\hrule
	\vspace{1.4mm}
	\begin{enumerate}[leftmargin=0pt,itemsep=2pt]
}{\end{enumerate}}
\newenvironment{decisions}{%
	\usection{Discussed points \& Decisions}
	\hrule
	\vspace{1.4mm}
	\begin{enumerate}[leftmargin=0pt,itemsep=2pt]
	\def\point{\item}
	\newcommand{\dec}[1]{\ul{##1}}
}{\end{enumerate}}
\newenvironment{todos}{%
	\usection{Open points}
	\hrule
	\vspace{1.4mm}
...
\usepackage{titlesec}
\titlespacing{\section}{0mm}{6mm}{1mm}
...
\usepackage{colortbl} % Support for colorint tables
